...
const PROMPTS = [
  { label: 'Tenant rights', prompt: 'What are my rights as a tenant?' },
  { label: 'Government schemes', prompt: 'Which government schemes am I eligible for?' },
  { label: 'RTI', prompt: 'How can I file an RTI?' },
  { label: 'Consumer complaint', prompt: 'How do I complain about a consumer issue?' },
]
    ...
    <div className="dashboard-page">
      ...
      <Card className="dashboard-primary-card">
        ...
        <div className="dashboard-primary-copy">
          <span className="eyebrow"><Sparkles size={14} /> Primary AI assistant</span>
          <h2>How can we help you today?</h2>
          <p>Describe a civic, legal or government-service problem and get a clear action plan.</p>
        </div>
        ...
        <div className="prompt-row" aria-label="Suggested prompts">
          {PROMPTS.map((item) => (
            <button key={item.label} className="prompt-chip" onClick={() => openAssistant(item.prompt)}>
              {item.label}
            </button>
          ))}
        </div>
      ...
      <div className="dashboard-tertiary-grid">
        ...
        <div className="dashboard-stat-grid">
          ...
          <StatCard icon={MessageCircle} value="8" label="AI Conversations" tint="var(--color-primary-tint)" color="var(--color-primary)" />
        </div>
      </div>
    </div>
  )
